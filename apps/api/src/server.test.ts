...
describe('bookmarks api', () => {
  ...
  it('creates and upserts bookmarks', async () => {
    ...
    const createResponse = await request(app).post('/api/bookmarks').send(payload).expect(201);
  ...
  it('paginates and searches bookmarks', async () => {
    ...
    await Promise.all(payloads.map((payload) => request(app).post('/api/bookmarks').send(payload)));

    const listResponse = await request(app).get('/api/bookmarks?page=1&limit=2').expect(200);
    expect(listResponse.body.items).toHaveLength(2);
    expect(listResponse.body.total).toBe(payloads.length);

    const searchResponse = await request(app).get('/api/bookmarks/search?q=Bookmark 1').expect(200);
  ...
  it('updates and deletes bookmarks', async () => {
    ...
    const created = await request(app).post('/api/bookmarks').send(payload).expect(201);

    await request(app).put(`/api/bookmarks/${created.body.id}`).send({ title: 'Renamed' }).expect(200);
    const updated = await request(app).get('/api/bookmarks').expect(200);
    expect(updated.body.items[0].title).toBe('Renamed');

    await request(app).delete(`/api/bookmarks/${created.body.id}`).expect(200);
    const afterDelete = await request(app).get('/api/bookmarks').expect(200);
